...
- **N5. Запись гейтится дважды:** `submit` вызывает `erp.write_invoice` **только** при `resolve_bool(ERP_WRITE_ENABLED)` (по умолчанию False); сам `EsuplErpProvider.write_invoice` **повторно гейтит** по тому же флагу (возвращает placeholder `esupl-prepared-<number>` без egress при OFF). Если тумблер выключился в гонке между двумя проверками, `submit` детектит placeholder (`is_write_disabled_placeholder`) и помечает накладную `failed`, а НЕ `written` с фейковым id (код-ревью 2026-07-21, L-3). См. [[erp-esupl-integration]] N4.
