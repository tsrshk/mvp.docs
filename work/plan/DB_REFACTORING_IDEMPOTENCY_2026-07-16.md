...
> **СТАТУС 2026-07-16: часть A РЕАЛИЗОВАНА и проверена** (BE pytest 281 ✓, в т.ч.
> 11 новых тестов идемпотентности; FE vitest 354 ✓, tsc ✓, build ✓; миграция `0014`
> применена, откат проверен; DEFER-04 снят, [[LCOS-F43-idempotency]] done).
> Отклонения от плана — см. примечание в A2.
>
> **СТАТУС 2026-07-20 (сессия 2026-07-16..20): часть B РЕАЛИЗОВАНА и проверена.**
> Миграции `0015_invoice_issued_at_tz` / `0016_supplier_price_sku_check` /
> `0017_email_lower_refresh_idx` применены к dev-БД, полный цикл отката проверен;
> BE pytest 285 ✓ (4 новых теста гигиены в `tests/test_schema_hygiene.py`).
> Отклонения: (1) ID ревизий сокращены — `alembic_version.version_num` varchar(32);
> (2) B5-чистка — только стартовый hook, БЕЗ суточного цикла (Phase-1 non-goal
> «нет фоновых задач», CLAUDE.md сильнее плана; в деплой-чеклист); (3) миграции
> 0016/0017 fail-closed на существующих нарушителях (падают с инструкцией, не чистят
> молча); (4) `Decimal(str(...))`-страховки в сервисах НЕ удалялись (безвредны,
> меньше диффа). Бонус-находка live-e2e: заголовок `Idempotency-Replayed` не был
> виден кросс-ориджин SPA — добавлен `expose_headers` в CORS (main.py).
...
> **Примечание по факту реализации.** Вместо строки-резервации в статусе `draft`
> (A2.1) реализован упрощённый эквивалент: INSERT идёт сразу с финальным пре-ERP
> статусом (rejected/validated/prepared) под SAVEPOINT, commit — строго ДО
> `erp.write_invoice` (инвариант сохранён). «В полёте» для проигравшего гонку —
> replay текущей prepared-строки, а не 409 (A2.4 не понадобился: нет тупика
> «вечный 409», нет нового enum-статуса). Конфликт тела — по
> `idempotency_request_hash` (A2.1 hash — как в плане). Легаси-путь без ключа
> остался однофазным (бит-в-бит прежняя семантика, FR-005).
