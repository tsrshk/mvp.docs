...
## 13. Уточнения по итогам ревью корректности (даблчек)

Инварианты, зафиксированные после адверсарного даблчека и покрытые тестами
(`tests/test_price_list_parse.py`, `test_supplier_sku_relations.py`, `test_supplier_assortment.py`):

- **Единый критерий `needs_review`.** Загрузка в `needs_review` ⇔ есть строка без
  разрешённого SKU (`resolution_method IS NULL`). Низкоуверенный авто-матч (fuzzy/ai с
  `confidence < price_list_confidence_min`) НЕ применяется автоматически — строка остаётся
  unresolved до ручного подтверждения. `parse` и пересчёт статуса в `patch_line`
  (`has_unresolved`) используют ОДИН критерий — статус не рассинхронизируется.
- **Проекция и `is_available`.** `project_price_list_line` держит наблюдение в
  `supplier_prices` ТОЛЬКО пока строка доступна (`is_available`), разрешена и цена > 0;
  иначе проекция удаляется. Снятая с продажи позиция исчезает и из книги цен, и из
  ассортимента. Обновление проекции — **на месте** (id стабилен): переразметка доступности
  не меняет `id` наблюдения и не переключает «текущую цену» при равных `observed_at`.
- **Отрицательная цена из OCR** обнуляется перед вставкой (не роняет `CHECK(price>=0)`;
  строка идёт в ручную проверку).
- **Ассортимент** берёт последнюю версию каждой позиции и прячет помеченные недоступными
  (позиция, снятая в свежей версии, не «воскресает» из старой).
- **Scope moat при ручной привязке прайса.** `patch_line._learn_mapping` пишет `sku_mapping`
  в **org-scope** (`method=manual`, `confirmed_by`) — читается и прайс-резолвером, и
  commit-путём накладной. `_learn_mapping`/перезапись `confirmed_by` вызываются ТОЛЬКО при
  реальной смене привязки (патч одного `is_available` их не трогает).

### Известные ограничения (backlog, не блокируют прогон)

- Снять привязку SKU через API нельзя (явный `null` неотличим от отсутствия поля в PATCH) —
  можно только переназначить на другой SKU.
- Если для (поставщик, текст) уже есть **subdivision**-маппинг с иным `pos_ingredient_id`,
  он имеет приоритет над org-правкой из прайса при переразборе в subdivision-контексте
  (ручная org-правка молча не применится) — редкий кейс, вне пилота-одной-subdivision.
