...
- **`suppliers`** (`OrganizationScopedMixin`, **int pk**) — org-wide. `external_id?` (indexed), `name`, `tax_id?`, and **supplier-card fields (F3-B1, migration `0006`):** `contact_name?`, `phone?`, `messenger?`, `delivery_terms? (Text)`, `min_order_amount? Numeric(14,2)`, `min_order_note?`, `is_active (bool, default true)` — soft-hide of stale suppliers. Unique `(organization_id, external_id)`; composite index `(organization_id, name)`.
