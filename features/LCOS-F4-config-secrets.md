...
2. **Несекретные runtime-настройки** — `system_settings` (KV в БД + белый список `REGISTRY`): `ai_provider`, имена моделей, `ai_vpn_enabled`, `module_*`, `erp_write_enabled`. Разрешение строго **БД (валидировано) → дефолт реестра**, никогда env.
