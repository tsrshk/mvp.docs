...
- (Запланировано) Append-only таблица `supplier_prices` (`OrganizationScopedMixin`, int pk): `supplier_id` (FK, CASCADE), `ingredient_id` (FK, CASCADE), `price Numeric(14,4)` за базовую единицу, `currency` (по умолчанию BYN), `raw_name` (как строка была названа в инвойсе), `unit`, `qty` (контекст наблюдения), `source_invoice_id` (FK, SET NULL, nullable — NULL = ручной), `observed_at` (дата инвойса, не `created_at`). Индексы на `(organization_id, ingredient_id, observed_at)` и `(organization_id, supplier_id)`.
...
- **Фичи:** [[LCOS-F17-supplier-cards]] (родительский справочник/карточка — построено), [[LCOS-F21-price-change-signal]] (читает предыдущую цену, записанную здесь, чтобы поднять предупреждение об изменении), [[LCOS-F10-invoice-status-machine]] (путь submit, триггерящий авто-сбор), [[LCOS-F13-sku-identity-resolver]] (записываются только строки с разрешённым SKU). Межпоставщиковое сравнение и импорт цен из Excel/PDF вне scope (Phase 2 / родственная E4).
