...
### Управление из приложения (REST, с 2026-07-21 — [[ADR-023]])
Помимо SQLAdmin, структуру и пользователей теперь можно вести прямо в приложении
(раздел «Управление», виден admin/superadmin):
- **superadmin** — создаёт/удаляет организации, любых пользователей (в т.ч. других
  superadmin), назначает роли в любой орг.
- **admin** — в пределах своей организации: CRUD подразделений и пользователей,
  назначение ролей admin/manager (сверху вниз). Новые организации не создаёт.
- Пароль, заданный админом, — **одноразовый**: система показывает его один раз, юзер при
  входе обязан сменить (`must_change_password`). «Сброс пароля» так же выдаёт одноразовый.
- Endpoints: `organizations`(+`/subdivisions`), `users`(+`/memberships`,
  `/reset-password`), `GET /roles`. Гейты — RBAC (`app/auth/rbac.py`).

### Роли — шпаргалка (3-ролевая модель, [[ADR-023]])
- **[[superadmin]]** = `users.is_superadmin = true` (платформенный флаг, режим Бога).
- **[[admin]]** = `memberships.role = admin` (org-level `subdivision_id IS NULL` ⇒ вся орг,
  либо конкретное подразделение). Управляет своей орг сверху вниз.
- **[[manager]]** = `memberships.role = manager` — прикладные фичи без управления людьми/настройками.
- **[[member]]** = любой пользователь с членством (общий термин).
