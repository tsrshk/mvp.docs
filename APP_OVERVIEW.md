...
- **Persist:** сопоставление каждой строки → SKU пишется в backend `sku_mapping` **отдельным клиентским** вызовом `POST /ingredients/mappings` (`method='manual'`, `confirmed_by` = аутентифицированный юзер), который FE делает в обработчике `onSend` (`widgets/invoice-workbench`) **до** мутации отправки (`sendInvoice`). Это отдельный запрос/транзакция, **а не сайд-эффект submit-эндпоинта**: сам submit `sku_mapping` не пишет — только читает его на commit-резолве. Отправка = подтверждение человеком. **Persist идёт ДО send** (persist-then-commit): подтверждённый mapping фиксируется раньше и в отдельной транзакции, поэтому переживает reject накладной by design — иначе fail-closed reject первой накладной не дал бы moat инициализироваться (`ADR-020`).
...
Миграции: `0001`…`0009` (последняя — `0009_sku_mapping_packing`) + `1e12…` (OCR-prompt в system_settings). Extension `vector` — в init-миграции; колонка `invoice_lines.sku_embedding` **не используется** — её никто не читает и не пишет, нет ANN-индекса, нет embedding-провайдера. Это неиспользуемая колонка-задел (будущее семантическое сопоставление), помеченная под чистку dead-code (`DEC-0011`; backlog `DEC-02`, `status: open`). Никакого триггера/механизма записи у неё нет.
