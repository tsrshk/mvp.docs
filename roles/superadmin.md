...
> **Обновление 2026-07-21 ([[ADR-023]]):** введена 3-ролевая модель (superadmin/[[admin]]/[[manager]]).
> superadmin ОСТАЁТСЯ платформенным флагом (не значением enum). Дополнительно: только
> superadmin создаёт/удаляет организации и выставляет флаг superadmin другим; управление
> структурой и пользователями теперь доступно и через app-REST (`organizations`/`users`
> роутеры, RBAC), не только через SQLAdmin operator ([[ADR-007]] — плоскости не смешиваются).
