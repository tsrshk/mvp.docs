...
**Plane:** app-plane (приложение, JWT) · **Identity:** `memberships.role = admin` (org-level
`subdivision_id IS NULL` ⇒ вся организация, либо конкретное подразделение).

> **Обновление 2026-07-21 ([[ADR-023]]):** введена 3-ролевая модель. enum `Role` теперь
> несёт `admin` и `manager` (см. [[manager]]). admin получил полноценное управление своей
> организацией через API: CRUD подразделений и пользователей, назначение ролей admin/manager
> вниз, настройки орг — в пределах своего орг-поддерева (сверху вниз). НЕ создаёт новые
> организации (только [[superadmin]]) и не выдаёт superadmin. Прежняя формулировка «RBAC —
> non-goal, единственное значение enum» снята этим ADR.
